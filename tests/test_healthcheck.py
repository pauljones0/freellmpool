...
def test_run_healthcheck_does_not_admit_unreviewed_discovered_model(monkeypatch):
    ...
    assert rows[0].ok
    assert rows[0].target == "aggregator/meta-llama/Meta-Llama-3.1-8B-Instruct"
    assert post.calls[0]["body"]["model"] == "meta-llama/Meta-Llama-3.1-8B-Instruct"
...
def test_run_healthcheck_honors_configured_pin_missing_from_discovery(monkeypatch):
    from freellmpool import benchmark
    ...
    assert post.calls[0]["body"]["model"] == "stale"
...
def test_run_healthcheck_failure_and_rate_limit(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "benchmark",
        lambda *args, **kwargs: [
            FakeRow("demo/a", "HTTP 429: Too Many Requests"),
            FakeRow("demo/b", "HTTP 500: Internal Error"),
        ],
    )
